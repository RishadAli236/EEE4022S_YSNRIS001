...
C=repmat(hamming(size(mf_no_clutter,1)),1,size(mf_no_clutter,2));
Rangedoppler=circshift(fft(C.*mf,[],1),31);
Rangedoppler_abs=20*log10(abs(Rangedoppler));
VelocityAxis = ((-1/2):1/num_pulses:1/2-1/num_pulses)*PRF*lamda/2*100;
clims=[max(Rangedoppler_abs(:))-10 max(Rangedoppler_abs(:))];
figure(2);imagesc(RangeAxis,VelocityAxis,Rangedoppler_abs,clims);colorbar;colormap('jet');
end
